...
<template>
  ...
  <form @submit.prevent="test">
    <div class="mb-3">
        ...
        <input v-model="store.NameSurname" type="text" name="user_name" id="name_surname" class="form-control" placeholder="Inserisci il tuo nome e cognome" />
    </div>
    <div class="mb-3">
        <label for="email" class="form-label">Indirizzo Email</label>
        <input v-model="store.UserEmail" type="email" name="user_email" id="email" class="form-control" placeholder="Inserisci la tua email" />
    </div>
    <div class="mb-3">
        <label for="email_title" class="form-label">Oggetto della mail</label>
        <input v-model="store.EmailTitle" type="text" name="email_title" id="email_title" class="form-control" placeholder="Oggetto della mail" />
    </div>
    <label for="email_text" class="form-label">Contenuto della mail</label>
    <textarea v-model="store.EmailMessage" name="description" id="email_text" class="form-control" placeholder="Scrivi un messaggio" style="width: 100%; height: 150px"></textarea>
